...
def prepare_day_info(d: pd.DataFrame, bounds: dict, lookback: int):
    x = d.copy()
    # Classic Turtle channel based on prior N daily highs/lows (no look-ahead).
    x["turtle_hi"] = x["High"].shift(1).rolling(lookback).max()
    x["turtle_lo"] = x["Low"].shift(1).rolling(lookback).min()
    x = x.dropna(subset=["pp", "r2", "s2", "turtle_hi", "turtle_lo"]).copy()
...
def build_entries(m: pd.DataFrame, info: dict):
    ...
    for d in sorted(info.keys()):
        ...
        # Turtle regime filter:
        # - BUY only when day open is in lower half of turtle channel.
        # - SELL only when day open is in upper half of turtle channel.
        t_hi = v["t_hi"]
        t_lo = v["t_lo"]
        if t_hi <= t_lo:
...
def main():
    d, m, bounds = load_data()
    rows = []
    for n in TURTLE_LOOKBACKS:
        info = prepare_day_info(d, bounds, n)
        pendings, st = build_entries(m, info)
        r = simulate(m, pendings, st)
        r["turtle_lookback"] = n
        rows.append(r)
        print(f"N={n:>2} cal_ppd={r['cal_ppd']:+.4f} wr={r['win_rate']:.4f} entries={r['entries']}")
    ...
    out = {
        ...
        "sl_norm": SL_NORM,
        "tp_norm": TP_NORM,
        "turtle_filter": "BUY only if day open <= turtle mid; SELL only if day open >= turtle mid",
        "lookbacks": TURTLE_LOOKBACKS,
        "best": df.iloc[0].to_dict(),
        "all_results": df.to_dict(orient="records"),
    }

    out_json = OUT_DIR / "pivot_zone_upto55_sl55_tp045_turtle_filter_sweep_2003_2015_eurjpy.json"
    out_csv = OUT_DIR / "pivot_zone_upto55_sl55_tp045_turtle_filter_sweep_2003_2015_eurjpy.csv"
