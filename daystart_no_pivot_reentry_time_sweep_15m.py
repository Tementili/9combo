...
def main():
    ...
    rows = []
    for h in range(24):
        rows.append(simulate_for_time(daily, m15, h, 0, pip))
    ...
    out = {
        ...
        "mode": "no-pivot-no-window-time-sweep",
    ...
    out_json = Path(__file__).parent / "daystart_no_pivot_reentry_time_sweep_15m_results.json"
    out_csv = Path(__file__).parent / "daystart_no_pivot_reentry_time_sweep_15m_results.csv"
