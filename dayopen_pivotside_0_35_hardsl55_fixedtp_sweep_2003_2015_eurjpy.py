...
def build_entries(info: dict, tp_norm: float):
    ...
    for d in sorted(info.keys()):
        ...
        if entry < pp:
            ...
            continue
        entries.append({"date": d, "i0": v["i0"], "side": side, "entry": entry, "sl": sl, "tp": tp})
    return entries
...
def run_one(m: pd.DataFrame, entries: list[dict]):
    ...
    for e in entries:
        ...
        p = close_pips(side, e["entry"], exit_px)
        total_pips += p
        if p > 0:
            wins += 1
        else:
...
def main():
    m, info = load_data()
    rows = []
    for tp in TP_NORM_GRID:
        entries = build_entries(info, tp)
        r = run_one(m, entries)
        r["tp_norm"] = tp
        rows.append(r)
        print(
            f"tp={tp:>4.2f} trades={r['trades']} wr={r['win_rate']:.4f} "
    ...
    out = {
        ...
        "risk_rule": "hard SL at 55% from pivot geometry + fixed TP",
    ...
    out_json = OUT_DIR / "dayopen_pivotside_0_35_hardsl55_fixedtp_sweep_2003_2015_eurjpy.json"
    out_csv = OUT_DIR / "dayopen_pivotside_0_35_hardsl55_fixedtp_sweep_2003_2015_eurjpy.csv"
