...
def run():
    ...
    summary = {
        "pair": "EURJPY",
        "strategy": f"SELL at PP when price drops to PP-S1 zone, 15m bars (ATR{ATR_PERIOD} sizing)",
        "sl_rule": f"fixed {SL_ATR_MULT}*ATR{ATR_PERIOD} ABOVE entry",
        "tp_rule": f"fixed {SL_ATR_MULT}*{TP_MULT}*ATR{ATR_PERIOD} BELOW entry (={SL_ATR_MULT * TP_MULT:.3f}*ATR{ATR_PERIOD})",
        "cost_pips": COST_PIPS,
    ...
    ledger_path = OUT_DIR / f"sell_pp_s1_fixed_sl70_tp91_atr{ATR_PERIOD}_15m_eurjpy_ledger.csv"
    summary_path = OUT_DIR / f"sell_pp_s1_fixed_sl70_tp91_atr{ATR_PERIOD}_15m_eurjpy_summary.json"
    df.drop(columns=["year"]).to_csv(ledger_path, index=False)
    summary_path.write_text(json.dumps(summary, indent=2))

    print(f"\n=== SELL at PP (price drops to PP-S1) | Fixed SL={SL_ATR_MULT}*ATR{ATR_PERIOD} | Fixed TP={SL_ATR_MULT * TP_MULT}*ATR{ATR_PERIOD} | 15m EURJPY ===")
