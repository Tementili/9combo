...
def run():
    ...
    for idx in range(1, len(daily)):
        ...
        # Only take SELL setups when the day opens under the pivot
        if not np.isfinite(day_open) or not np.isfinite(pp):
            continue
        if not (day_open < pp):
            continue

        if not np.isfinite(s1) or not np.isfinite(atr_val):
            continue
    ...
    summary = {
        "pair": "EURJPY",
        "strategy": "SELL at PP when price drops to PP-S1 zone, only if day opens under PP, ATR14 sizing",
        "sl_rule": f"fixed {SL_ATR_MULT}*ATR14 ABOVE entry",
        "tp_rule": f"fixed {SL_ATR_MULT}*{TP_MULT}*ATR14 BELOW entry (={SL_ATR_MULT * TP_MULT:.3f}*ATR14)",
        "cost_pips": COST_PIPS,
    ...
    ledger_path = OUT_DIR / "sell_pp_s1_fixed_sl70_tp91_atr14_open_underpp_15m_eurjpy_ledger.csv"
    summary_path = OUT_DIR / "sell_pp_s1_fixed_sl70_tp91_atr14_open_underpp_15m_eurjpy_summary.json"
    df.drop(columns=["year"]).to_csv(ledger_path, index=False)
    summary_path.write_text(json.dumps(summary, indent=2))

    print("\n=== SELL at PP (price drops to PP-S1) | Only if day opens UNDER PP | Fixed SL=0.70*ATR14 | Fixed TP=0.91*ATR14 ===")
    ...
    print(f"\nComparison:")
    print(f"  SELL baseline (no open filter):   see sell_pp_s1_fixed_sl70_tp91_15m_eurjpy_summary.json")
    print(f"  This filtered run:   {summary['total_pips']:+,.0f} pips  cal_ppd={summary['cal_ppd']:+.4f}  wr={summary['win_rate']:.2%}")
