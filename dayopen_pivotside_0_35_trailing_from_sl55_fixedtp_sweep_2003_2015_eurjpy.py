...
MIN_DENOM_PIPS = 10.0
TP_NORM_GRID = [round(x, 2) for x in np.arange(0.00, 1.01, 0.05)]
...
def build_entries(info: dict, tp_norm: float):
    ...
    for d in sorted(info.keys()):
        ...
        trail_dist = abs(entry - sl0)  # makes trailing start exactly at SL55 level
        entries.append({"date": d, "i0": v["i0"], "side": side, "entry": entry, "sl0": sl0, "tp": tp, "trail_dist": trail_dist})
    return entries


def run_one(m: pd.DataFrame, entries: list[dict]):
    ...
    for e in entries:
        ...
        if side == "BUY":
            ...
            for j in range(i0, len(hi)):
                best = max(best, hi[j])
                sl = max(sl, best - td)  # trailing stop only moves up
                hit_sl = lo[j] <= sl
                hit_tp = hi[j] >= tp
            ...
            for j in range(i0, len(hi)):
                best = min(best, lo[j])
                sl = min(sl, best + td)  # trailing stop only moves down
                hit_sl = hi[j] >= sl
                hit_tp = lo[j] <= tp
                if hit_sl and hit_tp:
                    px = sl
...
def main():
    ...
    for tp_norm in TP_NORM_GRID:
        ...
        rows.append(r)
        print(
            f"tp={tp_norm:>4.2f} wr={r['win_rate']:.4f} "
    ...
    out = {
        ...
        "risk_rule": "SL starts at 55% level and trails from there; fixed TP active",
    ...
    out_json = OUT_DIR / "dayopen_pivotside_0_35_trailing_from_sl55_fixedtp_sweep_2003_2015_eurjpy.json"
    out_csv = OUT_DIR / "dayopen_pivotside_0_35_trailing_from_sl55_fixedtp_sweep_2003_2015_eurjpy.csv"
