    ...
    for e in entries:
        ...
        if side == "BUY":
            ...
            for j in range(e["i0"], len(hi)):
                best = max(best, hi[j])
                tsl = best - trail_px
                sl = max(sl, tsl)  # never below hard SL
                if lo[j] <= sl:
                    px = sl
                    still_open = False
                    break
            ...
            for j in range(e["i0"], len(hi)):
                best = min(best, lo[j])
                tsl = best + trail_px
                sl = min(sl, tsl)  # never above hard SL
                if hi[j] >= sl:
                    px = sl
                    still_open = False
                    break
...
def main():
    ...
    out = {
        ...
        "risk_rule": "hard SL at 55% from pivot geometry + trailing stop",
    ...
    out_json = OUT_DIR / "dayopen_pivotside_0_35_hardsl55_trailing_sweep_2003_2015_eurjpy.json"
    out_csv = OUT_DIR / "dayopen_pivotside_0_35_hardsl55_trailing_sweep_2003_2015_eurjpy.csv"
