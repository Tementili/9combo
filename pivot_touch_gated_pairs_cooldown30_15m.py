...
MAX_HOLD_HOURS = 432
SPREAD_PIPS = 2.0
ENTRY_COOLDOWN_MIN = 30  # max one new pair per 30 minutes
...
def run() -> dict:
    ...
    cooldown_bars = max(1, ENTRY_COOLDOWN_MIN // 15)
    ...
    for j in range(len(times)):
        ...
        for p in open_positions:
            ...
            if j - p.open_bar >= MAX_HOLD_HOURS * 4:
        ...
        if touched_pivot:
            pivot_touch_count += 1
            if gate_open:
                if j - last_open_bar >= cooldown_bars:
                    pair_counter += 1
    ...
    out = {
        ...
        "timeframe": "15m",
        "mode": "daily_pivot_touch_gated_pairs_cooldown",
        ...
        "entry_cooldown_minutes": ENTRY_COOLDOWN_MIN,
        "sl_mult": SL_MULT,
        "tp_mult": TP_MULT,
    ...
    out_path = OUTPUT_DIR / f"pivot_touch_gated_pairs_cooldown30_{PAIR}_{START_DATE}_{END_DATE}.json"
