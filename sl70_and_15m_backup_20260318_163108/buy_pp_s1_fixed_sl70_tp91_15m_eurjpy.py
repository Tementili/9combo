...
def run():
    ...
    for idx in range(1, len(daily)):
        ...
        for j in range(i_start, i_end + 1):
            bar_low = m15_lows[j]
            bar_high = m15_highs[j]
            if bar_low <= pp and bar_high >= s1:
                entry_px = max(min(pp, float(m15_closes[j])), s1)
                if bar_low <= pp:
                    entry_px = pp
                entry_bar = j
                break
    ...
    cal_ppd = total_pips / cal_days if cal_days > 0 else 0
    ...
    summary = {
        "pair": "EURJPY",
        "strategy": "BUY between PP and S1 on 15m bars",
        "sl_rule": f"fixed {SL_ATR_MULT}*ATR14 below entry",
        "tp_rule": f"fixed {SL_ATR_MULT}*{TP_MULT}*ATR14 above entry (={SL_ATR_MULT * TP_MULT:.3f}*ATR14)",
        "cost_pips": COST_PIPS,
        ...
        "avg_pips_per_trade": round(total_pips / n_trades, 4) if n_trades > 0 else 0,
    ...
    ledger_path = OUT_DIR / "buy_pp_s1_fixed_sl70_tp91_15m_eurjpy_ledger.csv"
    summary_path = OUT_DIR / "buy_pp_s1_fixed_sl70_tp91_15m_eurjpy_summary.json"
    df.drop(columns=["year"]).to_csv(ledger_path, index=False)
    summary_path.write_text(json.dumps(summary, indent=2))

    print("\n=== BUY PP-to-S1 | Fixed SL=0.70*ATR14 | Fixed TP=0.91*ATR14 | 15m EURJPY ===")
