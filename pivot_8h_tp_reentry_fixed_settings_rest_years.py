...
# Same logic, just evaluating all remaining year blocks.
PERIODS = [
    ("reference_2014_2021", "2014-01-01", "2021-12-31"),
    ("rest_pre_2014", "2003-08-04", "2013-12-31"),
    ("rest_2022_plus", "2022-01-01", "2026-03-06"),
    ("full_available", "2003-08-04", "2026-03-06"),
]
...
def new_buy(entry: float, sl_dist: float, tp_dist: float, bar: int, active_from: int) -> Pos:
    return Pos("BUY", entry, entry + tp_dist, entry - sl_dist, entry, bar, active_from)


def new_sell(entry: float, sl_dist: float, tp_dist: float, bar: int, active_from: int) -> Pos:
    return Pos("SELL", entry, entry - tp_dist, entry + sl_dist, entry, bar, active_from)
...
def main() -> None:
    ...
    _ = opens  # Unused but kept for symmetry with loader return values.
    atr = atr_wilder(
    ...
    for name, s, e in PERIODS:
        results[name] = run_period(daily, times, highs, lows, closes, atr, s, e)

    out = {
        "pair": PAIR.upper(),
        "timeframe": "15m",
        "mode": "8h_open_tp_reentry_chain",
        "window_starts_utc": WINDOW_STARTS,
        "atr_source": "daily ATR14 (previous day)",
        "max_hold_hours": MAX_HOLD_HOURS,
        "results": results,
    }
    out_path = OUTPUT_DIR / "pivot_8h_tp_reentry_fixed_settings_rest_years_usdjpy.json"
    out_path.write_text(json.dumps(out, indent=2), encoding="utf-8")

    print("Results by period:")
    for k, v in results.items():
        print(
            f"{k:18s}  {v['period'][0]}..{v['period'][1]}  "
            f"cal_ppd={v['cal_ppd']:+.4f}  total_pips={v['total_pips']:+.1f}"
        )
