...
def run():
    ...
    summary = {
        "pair": "EURJPY",
        "strategy": "SELL at PP when price drops to PP-S1 zone, 15m bars",
    ...
    ledger_path = OUT_DIR / "sell_pp_s1_fixed_sl70_tp91_15m_eurjpy_ledger.csv"
    summary_path = OUT_DIR / "sell_pp_s1_fixed_sl70_tp91_15m_eurjpy_summary.json"
    df.drop(columns=["year"]).to_csv(ledger_path, index=False)
    summary_path.write_text(json.dumps(summary, indent=2))

    print("\n=== SELL at PP (price drops to PP-S1) | Fixed SL=0.70*ATR14 | Fixed TP=0.91*ATR14 | 15m EURJPY ===")
